 ...
 * protocol (see example_plugins/TABAME_PLUGIN_SKILL.md). Type the keyword `tw`
...
const FRACTIONS = {
  ...
  '1/6': '16.666667%', '2/6': '33.333333%', '3/6': '50%', '4/6': '66.666667%',
  '5/6': '83.333333%', full: '100%',
};
...
const SPECIAL_COLORS = { inherit: 'inherit', current: 'currentColor', transparent: 'transparent', black: '#000000', white: '#ffffff' };
...
function addScale(prefix, props, scale, cat, doc, { negatives = false, join = ' ' } = {}) {
  for (const key of Object.keys(scale)) {
    const val = scale[key];
    const decls = props.map((p) => `${p}: ${val};`).join(join);
    add(`${prefix}-${key}`, decls, cat, doc);
    if (negatives && val !== '0px' && val !== 'auto' && !String(val).startsWith('-')) {
...
addStatic([
  ...
  ['inline-grid', 'display: inline-grid;'], ['table', 'display: table;'], ['inline-table', 'display: inline-table;'],
  ['contents', 'display: contents;'], ['flow-root', 'display: flow-root;'], ['list-item', 'display: list-item;'],
  ['hidden', 'display: none;'],
], 'Layout', 'display');
addStatic([
...
addStatic([
  ['overflow-auto', 'overflow: auto;'], ['overflow-hidden', 'overflow: hidden;'], ['overflow-clip', 'overflow: clip;'],
...
addStatic([
  ...
  ['object-none', 'object-fit: none;'], ['object-scale-down', 'object-fit: scale-down;'],
], 'Layout', 'object-fit');
addStatic([
  ['box-border', 'box-sizing: border-box;'], ['box-content', 'box-sizing: content-box;'],
], 'Layout', 'box-sizing');
addStatic([
  ['float-start', 'float: inline-start;'], ['float-end', 'float: inline-end;'], ['float-right', 'float: right;'],
  ['float-left', 'float: left;'], ['float-none', 'float: none;'],
], 'Layout', 'float');
addStatic([
...
addStatic([
  ...
  ['z-40', 'z-index: 40;'], ['z-50', 'z-index: 50;'], ['z-auto', 'z-index: auto;'],
], 'Layout', 'z-index');

// Inset (top / right / bottom / left)
const INSET = { ...SPACING, auto: 'auto', '1/2': '50%', '1/3': '33.333333%', '2/3': '66.666667%', '1/4': '25%', '3/4': '75%', full: '100%' };
addScale('inset', ['inset'], INSET, 'Layout', 'top-right-bottom-left', { negatives: true });
addScale('inset-x', ['left', 'right'], INSET, 'Layout', 'top-right-bottom-left', { negatives: true });
addScale('inset-y', ['top', 'bottom'], INSET, 'Layout', 'top-right-bottom-left', { negatives: true });
addScale('top', ['top'], INSET, 'Layout', 'top-right-bottom-left', { negatives: true });
addScale('right', ['right'], INSET, 'Layout', 'top-right-bottom-left', { negatives: true });
addScale('bottom', ['bottom'], INSET, 'Layout', 'top-right-bottom-left', { negatives: true });
addScale('left', ['left'], INSET, 'Layout', 'top-right-bottom-left', { negatives: true });
...
addStatic([
  ...
  ['justify-self-end', 'justify-self: end;'], ['justify-self-center', 'justify-self: center;'], ['justify-self-stretch', 'justify-self: stretch;'],
...
addStatic([
  ['content-normal', 'align-content: normal;'], ['content-center', 'align-content: center;'], ['content-start', 'align-content: flex-start;'],
  ['content-end', 'align-content: flex-end;'], ['content-between', 'align-content: space-between;'],
  ['content-around', 'align-content: space-around;'], ['content-evenly', 'align-content: space-evenly;'], ['content-stretch', 'align-content: stretch;'],
...
addStatic([
  ...
  ['place-content-center', 'place-content: center;'], ['place-content-start', 'place-content: start;'],
  ['place-content-end', 'place-content: end;'], ['place-content-between', 'place-content: space-between;'],
...
// grid columns / rows
for (let n = 1; n <= 12; n++) add(`grid-cols-${n}`, `grid-template-columns: repeat(${n}, minmax(0, 1fr));`, 'Flexbox & Grid', 'grid-template-columns');
add('grid-cols-none', 'grid-template-columns: none;', 'Flexbox & Grid', 'grid-template-columns');
add('grid-cols-subgrid', 'grid-template-columns: subgrid;', 'Flexbox & Grid', 'grid-template-columns');
for (let n = 1; n <= 6; n++) add(`grid-rows-${n}`, `grid-template-rows: repeat(${n}, minmax(0, 1fr));`, 'Flexbox & Grid', 'grid-template-rows');
add('grid-rows-none', 'grid-template-rows: none;', 'Flexbox & Grid', 'grid-template-rows');
for (let n = 1; n <= 12; n++) add(`col-span-${n}`, `grid-column: span ${n} / span ${n};`, 'Flexbox & Grid', 'grid-column');
add('col-auto', 'grid-column: auto;', 'Flexbox & Grid', 'grid-column');
add('col-span-full', 'grid-column: 1 / -1;', 'Flexbox & Grid', 'grid-column');
for (let n = 1; n <= 13; n++) add(`col-start-${n}`, `grid-column-start: ${n};`, 'Flexbox & Grid', 'grid-column');
for (let n = 1; n <= 13; n++) add(`col-end-${n}`, `grid-column-end: ${n};`, 'Flexbox & Grid', 'grid-column');
for (let n = 1; n <= 6; n++) add(`row-span-${n}`, `grid-row: span ${n} / span ${n};`, 'Flexbox & Grid', 'grid-row');
...
addStatic([
  ...
  ['grid-flow-dense', 'grid-auto-flow: dense;'], ['grid-flow-row-dense', 'grid-auto-flow: row dense;'],
  ['grid-flow-col-dense', 'grid-auto-flow: column dense;'],
], 'Flexbox & Grid', 'grid-auto-flow');
addStatic([
  ['order-first', 'order: -9999;'], ['order-last', 'order: 9999;'], ['order-none', 'order: 0;'],
], 'Flexbox & Grid', 'order');
for (let n = 1; n <= 12; n++) add(`order-${n}`, `order: ${n};`, 'Flexbox & Grid', 'order');
// gap / space-between
...
for (const key of Object.keys(SPACING)) {
  add(`space-x-${key}`, `> :not([hidden]) ~ :not([hidden]) { margin-left: ${SPACING[key]}; }`, 'Flexbox & Grid', 'space');
  add(`space-y-${key}`, `> :not([hidden]) ~ :not([hidden]) { margin-top: ${SPACING[key]}; }`, 'Flexbox & Grid', 'space');
}

// ---- Spacing (padding / margin) --------------------------------------------
const PAD_SIDES = { '': ['padding'], x: ['padding-left', 'padding-right'], y: ['padding-top', 'padding-bottom'], t: ['padding-top'], r: ['padding-right'], b: ['padding-bottom'], l: ['padding-left'], s: ['padding-inline-start'], e: ['padding-inline-end'] };
for (const s of Object.keys(PAD_SIDES)) addScale(`p${s}`, PAD_SIDES[s], SPACING, 'Spacing', 'padding');
const MAR = { ...SPACING, auto: 'auto' };
const MAR_SIDES = { '': ['margin'], x: ['margin-left', 'margin-right'], y: ['margin-top', 'margin-bottom'], t: ['margin-top'], r: ['margin-right'], b: ['margin-bottom'], l: ['margin-left'], s: ['margin-inline-start'], e: ['margin-inline-end'] };
for (const s of Object.keys(MAR_SIDES)) addScale(`m${s}`, MAR_SIDES[s], MAR, 'Spacing', 'margin', { negatives: true });

// ---- Sizing (width / height) ------------------------------------------------
const W = { ...SPACING, ...FRACTIONS, auto: 'auto', screen: '100vw', svw: '100svw', lvw: '100lvw', dvw: '100dvw', min: 'min-content', max: 'max-content', fit: 'fit-content' };
addScale('w', ['width'], W, 'Sizing', 'width');
const H = { ...SPACING, '1/2': '50%', '1/3': '33.333333%', '2/3': '66.666667%', '1/4': '25%', '3/4': '75%', full: '100%', auto: 'auto', screen: '100vh', svh: '100svh', lvh: '100lvh', dvh: '100dvh', min: 'min-content', max: 'max-content', fit: 'fit-content' };
addScale('h', ['height'], H, 'Sizing', 'height');
const SIZE = { ...SPACING, ...FRACTIONS, auto: 'auto', min: 'min-content', max: 'max-content', fit: 'fit-content' };
addScale('size', ['width', 'height'], SIZE, 'Sizing', 'size');
addScale('min-w', ['min-width'], { 0: '0px', full: '100%', min: 'min-content', max: 'max-content', fit: 'fit-content' }, 'Sizing', 'min-width');
addScale('min-h', ['min-height'], { 0: '0px', full: '100%', screen: '100vh', svh: '100svh', lvh: '100lvh', dvh: '100dvh', min: 'min-content', max: 'max-content', fit: 'fit-content' }, 'Sizing', 'min-height');
const MAXW = { 0: '0rem', none: 'none', xs: '20rem', sm: '24rem', md: '28rem', lg: '32rem', xl: '36rem', '2xl': '42rem', '3xl': '48rem', '4xl': '56rem', '5xl': '64rem', '6xl': '72rem', '7xl': '80rem', full: '100%', min: 'min-content', max: 'max-content', fit: 'fit-content', prose: '65ch' };
addScale('max-w', ['max-width'], MAXW, 'Sizing', 'max-width');
addScale('max-h', ['max-height'], { ...SPACING, none: 'none', full: '100%', screen: '100vh', min: 'min-content', max: 'max-content', fit: 'fit-content' }, 'Sizing', 'max-height');
...
for (const key of Object.keys(FONT_SIZE)) {
  ...
  add(`text-${key}`, `font-size: ${fs}; line-height: ${lh};`, 'Typography', 'font-size');
}
addStatic([
...
addStatic([
  ...
  ['whitespace-pre', 'white-space: pre;'], ['whitespace-pre-line', 'white-space: pre-line;'], ['whitespace-pre-wrap', 'white-space: pre-wrap;'],
], 'Typography', 'whitespace');
addStatic([
  ['break-normal', 'overflow-wrap: normal; word-break: normal;'], ['break-words', 'overflow-wrap: break-word;'],
...
for (const n of [3, 4, 5, 6, 7, 8, 9, 10]) add(`leading-${n}`, `line-height: ${SPACING[n]};`, 'Typography', 'line-height');
addStatic([
  ['align-baseline', 'vertical-align: baseline;'], ['align-top', 'vertical-align: top;'], ['align-middle', 'vertical-align: middle;'],
  ['align-bottom', 'vertical-align: bottom;'], ['align-text-top', 'vertical-align: text-top;'], ['align-text-bottom', 'vertical-align: text-bottom;'],
], 'Typography', 'vertical-align');
addStatic([
  ['decoration-solid', 'text-decoration-style: solid;'], ['decoration-double', 'text-decoration-style: double;'],
  ['decoration-dotted', 'text-decoration-style: dotted;'], ['decoration-dashed', 'text-decoration-style: dashed;'], ['decoration-wavy', 'text-decoration-style: wavy;'],
...
const COLOR_DOC = {
  bg: 'background-color', text: 'text-color', border: 'border-color', ring: 'ring-color', divide: 'divide-color',
  outline: 'outline-color', decoration: 'text-decoration-color', caret: 'caret-color', accent: 'accent-color',
  fill: 'fill', stroke: 'stroke', from: 'gradient-color-stops', via: 'gradient-color-stops', to: 'gradient-color-stops',
...
function addColor(prefix, name, hex) {
  ...
  const css = `${prop}: ${hex};`;
  add(`${prefix}-${name}`, css, colorCat(prefix), COLOR_DOC[prefix], { hex: hex.startsWith('#') ? hex : undefined });
}
for (const prefix of Object.keys(COLOR_PREFIXES)) {
  for (const [name, hex] of Object.entries(SPECIAL_COLORS)) addColor(prefix, name, hex);
  for (const color of Object.keys(PALETTE)) {
    for (const shade of Object.keys(PALETTE[color])) addColor(prefix, `${color}-${shade}`, PALETTE[color][shade]);
  }
}
// text-color also owns the font-size classes above; opacity helpers:
for (const o of [0, 5, 10, 20, 25, 30, 40, 50, 60, 70, 75, 80, 90, 95, 100]) add(`opacity-${o}`, `opacity: ${o / 100};`, 'Effects', 'opacity');
...
addStatic([
  ...
  ['bg-cover', 'background-size: cover;'], ['bg-contain', 'background-size: contain;'], ['bg-auto', 'background-size: auto;'],
  ['bg-repeat', 'background-repeat: repeat;'], ['bg-no-repeat', 'background-repeat: no-repeat;'], ['bg-repeat-x', 'background-repeat: repeat-x;'],
  ['bg-center', 'background-position: center;'], ['bg-top', 'background-position: top;'], ['bg-bottom', 'background-position: bottom;'],
], 'Backgrounds', 'background-size');

// ---- Borders ----------------------------------------------------------------
const RADIUS = { none: '0px', sm: '0.125rem', '': '0.25rem', md: '0.375rem', lg: '0.5rem', xl: '0.75rem', '2xl': '1rem', '3xl': '1.5rem', full: '9999px' };
const RADIUS_SIDES = { '': ['border-radius'], t: ['border-top-left-radius', 'border-top-right-radius'], r: ['border-top-right-radius', 'border-bottom-right-radius'], b: ['border-bottom-right-radius', 'border-bottom-left-radius'], l: ['border-top-left-radius', 'border-bottom-left-radius'], tl: ['border-top-left-radius'], tr: ['border-top-right-radius'], br: ['border-bottom-right-radius'], bl: ['border-bottom-left-radius'] };
for (const side of Object.keys(RADIUS_SIDES)) {
  for (const key of Object.keys(RADIUS)) {
    ...
    const css = RADIUS_SIDES[side].map((p) => `${p}: ${RADIUS[key]};`).join(' ');
    add(cls, css, 'Borders', 'border-radius');
  }
}
const BW = { '': '1px', 0: '0px', 2: '2px', 4: '4px', 8: '8px' };
const BW_SIDES = { '': ['border-width'], x: ['border-left-width', 'border-right-width'], y: ['border-top-width', 'border-bottom-width'], t: ['border-top-width'], r: ['border-right-width'], b: ['border-bottom-width'], l: ['border-left-width'], s: ['border-inline-start-width'], e: ['border-inline-end-width'] };
...
addStatic([
  ...
  ['border-double', 'border-style: double;'], ['border-hidden', 'border-style: hidden;'], ['border-none', 'border-style: none;'],
...
addStatic([
  ...
  ['outline-0', 'outline-width: 0px;'], ['outline-1', 'outline-width: 1px;'], ['outline-2', 'outline-width: 2px;'],
  ['outline-4', 'outline-width: 4px;'], ['outline-8', 'outline-width: 8px;'],
], 'Borders', 'outline-width');
addStatic([
  ['ring', 'box-shadow: 0 0 0 3px var(--tw-ring-color);'], ['ring-0', 'box-shadow: 0 0 0 0px var(--tw-ring-color);'],
  ['ring-1', 'box-shadow: 0 0 0 1px var(--tw-ring-color);'], ['ring-2', 'box-shadow: 0 0 0 2px var(--tw-ring-color);'],
  ['ring-4', 'box-shadow: 0 0 0 4px var(--tw-ring-color);'], ['ring-8', 'box-shadow: 0 0 0 8px var(--tw-ring-color);'],
...
addStatic([
  ...
  ['sepia-0', 'filter: sepia(0);'], ['sepia', 'filter: sepia(100%);'],
...
for (const b of [0, 50, 75, 90, 95, 100, 105, 110, 125, 150, 200]) add(`brightness-${b}`, `filter: brightness(${b / 100});`, 'Filters', 'brightness');
for (const c of [0, 50, 75, 100, 125, 150, 200]) add(`contrast-${c}`, `filter: contrast(${c / 100});`, 'Filters', 'contrast');
...
for (const d of [0, 75, 100, 150, 200, 300, 500, 700, 1000]) add(`duration-${d}`, `transition-duration: ${d}ms;`, 'Transitions & Animation', 'transition-duration');
for (const d of [0, 75, 100, 150, 200, 300, 500, 700, 1000]) add(`delay-${d}`, `transition-delay: ${d}ms;`, 'Transitions & Animation', 'transition-delay');
addStatic([
...
for (const key of Object.keys(TRANSLATE)) {
  add(`translate-x-${key}`, `transform: translateX(${TRANSLATE[key]});`, 'Transforms', 'translate');
  add(`translate-y-${key}`, `transform: translateY(${TRANSLATE[key]});`, 'Transforms', 'translate');
...
addStatic([
  ['origin-center', 'transform-origin: center;'], ['origin-top', 'transform-origin: top;'], ['origin-bottom', 'transform-origin: bottom;'],
  ['origin-left', 'transform-origin: left;'], ['origin-right', 'transform-origin: right;'], ['origin-top-left', 'transform-origin: top left;'],
  ['transform-gpu', 'transform: translate3d(0,0,0);'], ['transform-none', 'transform: none;'],
...
addStatic([
  ...
  ['snap-start', 'scroll-snap-align: start;'], ['snap-center', 'scroll-snap-align: center;'], ['snap-end', 'scroll-snap-align: end;'],
  ['snap-x', 'scroll-snap-type: x var(--tw-scroll-snap-strictness);'], ['snap-y', 'scroll-snap-type: y var(--tw-scroll-snap-strictness);'],
  ['snap-mandatory', '--tw-scroll-snap-strictness: mandatory;'], ['snap-none', 'scroll-snap-type: none;'],
...
addStatic([
  ...
  ['container', 'width: 100%; /* + responsive max-widths at each breakpoint */'],
...
const CAT_ICON = {
  Layout: 'window', 'Flexbox & Grid': 'grid', Spacing: 'grid', Sizing: 'grid', Typography: 'label',
  Backgrounds: 'palette', Borders: 'window', Effects: 'star', Filters: 'brush',
  'Transitions & Animation': 'bolt', Transforms: 'refresh', Interactivity: 'app', SVG: 'image',
};
...
function searchCatalog(rawTerm) {
  ...
  scored.sort((a, b) => (b.s !== a.s ? b.s - a.s : a.entry.cls.length - b.entry.cls.length || a.entry.cls.localeCompare(b.entry.cls)));
  return scored.slice(0, 60).map((x) => x.entry);
}

// A short list shown when the query is empty, so the plugin is browsable.
const STARTER = ['flex', 'grid', 'hidden', 'block', 'items-center', 'justify-center', 'gap-4', 'p-4', 'px-4', 'm-4', 'mx-auto', 'w-full', 'h-screen', 'text-center', 'text-sm', 'font-bold', 'text-white', 'bg-blue-500', 'rounded-lg', 'border', 'shadow-md', 'transition', 'cursor-pointer', 'absolute', 'relative', 'overflow-hidden', 'grid-cols-3', 'space-y-2'];

// ── rendering ─────────────────────────────────────────────────────────────────
const itemsById = {}; // id -> { cls, css, url }

function previewMarkdown(entry) {
  const cssPretty = entry.css.split(/;\s+(?=[a-z-]+:)/).map((d) => (d.trim().endsWith(';') || d.trim().startsWith('>') || d.trim().startsWith('/') ? d.trim() : `${d.trim()};`)).join('\n  ');
...
function renderQuery(rev, text) {
  ...
  const entries = term ? searchCatalog(term) : STARTER.map((c) => CATALOG.find((e) => e.cls === c)).filter(Boolean);
  render(rev, 'list', {
    items: entries.map(toItem),
    preview: { enabled: true },
    emptyText: term ? `No Tailwind utility matches “${term}”` : 'Type a class name or CSS property',
  });
}
...
function handleLine(line) {
  ...
  try {
    switch (msg.type) {
      ...
      case 'init':
      case 'query':
        renderQuery(msg.rev || 0, msg.text != null ? msg.text : msg.query || '');
        break;
      case 'action':
    ...
    detailFrame(msg && msg.rev ? msg.rev : 0, `# Error\n\n\`\`\`\n${err && err.stack ? err.stack : err}\n\`\`\``);
  }
}

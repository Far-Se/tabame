...
static void ReadTrayToolbar(HWND toolbarWnd, bool isOverflow,
  ...
  LPVOID pRemote = VirtualAllocEx(hProc, nullptr, sizeof(TBBUTTON), MEM_COMMIT,
                                  PAGE_READWRITE);
  if (!pRemote) {
    CloseHandle(hProc);
    return;
  ...
  for (int i = 0; i < count; ++i) {
    ...
    TRAYDATA_LOCAL td{};
    if (!ReadProcessMemory(hProc, reinterpret_cast<LPCVOID>(tbb.dwData), &td,
                           sizeof(td), nullptr))
      continue;
...
std::vector<ExtTrayIcon> EnumAllTrayIcons() {
  ...
  if (overflowWnd) {
    HWND toolbar =
        FindWindowExW(overflowWnd, nullptr, L"ToolbarWindow32", nullptr);
...
static HWND GetTrayToolbarHwnd(bool overflow) {
  if (overflow) {
    ...
    return ow ? FindWindowExW(ow, nullptr, L"ToolbarWindow32", nullptr)
              : nullptr;
  }
  HWND main = FindWindowW(L"Shell_TrayWnd", nullptr);
  HWND notify =
      main ? FindWindowExW(main, nullptr, L"TrayNotifyWnd", nullptr) : nullptr;
  HWND pager =
      notify ? FindWindowExW(notify, nullptr, L"SysPager", nullptr) : nullptr;
  return pager ? FindWindowExW(pager, nullptr, L"ToolbarWindow32", nullptr)
               : nullptr;
}

// Find the UIA button element whose Name matches tipName inside toolbarHwnd.
// Caller owns the returned element (must Release()).
static IUIAutomationElement *FindTrayButtonByName(IUIAutomation *pAuto,
                                                  HWND toolbarHwnd,
                                                  const std::wstring &tipName) {
  if (!toolbarHwnd || !pAuto)
    return nullptr;
...
bool ClickTrayIconUia(const std::wstring &tipName, bool preferOverflow,
  ...
  if (clickType == 0 || clickType == 3) // left / double — use InvokePattern
  {
    ...
    if (pRaw) {
      ...
      if (SUCCEEDED(
              pRaw->QueryInterface(__uuidof(IUIAutomationInvokePattern),
    ...
    if (!ok) {
      ...
      if (pAcc) {
        ...
        if (SUCCEEDED(pAcc->QueryInterface(
                __uuidof(IUIAutomationLegacyIAccessiblePattern),
                reinterpret_cast<void **>(&pLeg))) &&
            pLeg) {
  ...
  } else // right (1) or middle (2) — resolve screen rect and SendInput
  {
    ...
    if (rect.right > rect.left && rect.bottom > rect.top) {
      ...
      DWORD downFlag =
          (clickType == 1) ? MOUSEEVENTF_RIGHTDOWN : MOUSEEVENTF_MIDDLEDOWN;
      DWORD upFlag =
          (clickType == 1) ? MOUSEEVENTF_RIGHTUP : MOUSEEVENTF_MIDDLEUP;

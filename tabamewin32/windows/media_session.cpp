  ...
  try {
    ...
    for (uint32_t index = 0; index < sessionCount; ++index) {
      ...
      const std::string sessionId =
          HStringToString(session.SourceAppUserModelId());
      entry[EncodableValue("id")] = EncodableValue(sessionId);
      entry[EncodableValue("isCurrent")] =
          EncodableValue(sessionId == currentId);
      ...
      entry[EncodableValue("canPlay")] =
          EncodableValue(controls.IsPlayEnabled());
      entry[EncodableValue("canPause")] =
          EncodableValue(controls.IsPauseEnabled());
      entry[EncodableValue("canSkipNext")] =
          EncodableValue(controls.IsNextEnabled());
      entry[EncodableValue("canSkipPrevious")] =
          EncodableValue(controls.IsPreviousEnabled());

      try {
        const auto props = session.TryGetMediaPropertiesAsync().get();
        if (props) {
          entry[EncodableValue("title")] =
              EncodableValue(HStringToString(props.Title()));
          entry[EncodableValue("artist")] =
              EncodableValue(HStringToString(props.Artist()));
          entry[EncodableValue("albumTitle")] =
              EncodableValue(HStringToString(props.AlbumTitle()));
          entry[EncodableValue("albumArtist")] =
              EncodableValue(HStringToString(props.AlbumArtist()));
...
flutter::EncodableValue MediaSessionCommand(const std::string &sessionId,
  ...
  auto makeError = [](const std::string &msg) -> EncodableValue {
    return EncodableValue(EncodableMap{
        {EncodableValue("error"), EncodableValue(msg)},
    });
  };
  ...
  try {
    ...
    if (!targetSession)
      return makeError("Session not found: " + sessionId);

    if (command == "play") {
      targetSession.TryPlayAsync().get();
    } else if (command == "pause") {
      targetSession.TryPauseAsync().get();
    } else if (command == "skipNext") {
      targetSession.TrySkipNextAsync().get();
    } else if (command == "skipPrevious") {
      targetSession.TrySkipPreviousAsync().get();
    } else if (command == "togglePlayPause") {
      targetSession.TryTogglePlayPauseAsync().get();
    } else if (command == "stop") {
      targetSession.TryStopAsync().get();
    } else {
    ...
    oss << "WinRT error 0x" << std::hex
        << static_cast<uint32_t>(ex.code().value) << ": "
        << HStringToString(ex.message());
    return makeError(oss.str());
  }

  return EncodableValue(); // null == success
}
} // namespace MediaSession

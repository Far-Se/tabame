...
template <typename T> void ReleaseIfSet(T *&ptr) {
  if (ptr) {
    ptr->Release();
    ptr = nullptr;
  }
...
bool IsRgbBlackFrame(const std::vector<uint8_t> &pixels, int width,
                     int height) {
...
bool CreateD3D11Device(ID3D11Device **device, ID3D11DeviceContext **context) {
...
bool CopyTextureToPixels(ID3D11Device *device, ID3D11DeviceContext *context,
...
bool CopyCaptureFrameToPixels(ID3D11Device *device,
  ...
  HRESULT hr =
      access->GetInterface(__uuidof(ID3D11Texture2D), sourceTexture.put_void());
  if (FAILED(hr))
    return false;
...
bool TryCaptureMonitorWithGraphicsCapture(HMONITOR monitor,
                                          MonitorCaptureResult &result) {
  if (monitor == nullptr)
    return false;
  ...
  try {
    ...
    Direct3D11CaptureFramePool framePool =
        Direct3D11CaptureFramePool::CreateFreeThreaded(
            direct3dDevice, DirectXPixelFormat::B8G8R8A8UIntNormalized, 1,
            size);
    ...
    auto token = framePool.FrameArrived(
        [&](auto const &, auto const &) { SetEvent(frameEvent); });
...
struct MonitorCaptureSession {
  ...
  bool CopyFrameToCache(IDXGIResource *desktopResource) {
    ...
    HRESULT hr = desktopResource->QueryInterface(
        __uuidof(ID3D11Texture2D), reinterpret_cast<void **>(&gpuTexture));
    if (FAILED(hr))
      goto fail;

    gpuTexture->GetDesc(&desc);

    if (!stagingTexture || stagingDesc.Width != desc.Width ||
    ...
    for (int y = 0; y < height; y++) {
      memcpy(framePixels.data() + static_cast<size_t>(y) * width * 4,
             static_cast<uint8_t *>(mapped.pData) +
                 static_cast<size_t>(y) * mapped.RowPitch,
  ...
  bool Capture(MonitorCaptureResult &result) {
    ...
    for (int attempt = 0; attempt < 8; attempt++) {
      ...
      HRESULT hr =
          duplication->AcquireNextFrame(16, &frameInfo, &desktopResource);
...
bool CaptureMonitorBitmapAlternative(int64_t monitorHandle,
                                     MonitorCaptureResult &result) {
  return TryCaptureMonitorWithGraphicsCapture(
      reinterpret_cast<HMONITOR>(static_cast<LONG_PTR>(monitorHandle)), result);
}

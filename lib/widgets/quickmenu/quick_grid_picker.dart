...
class QuickGridsPicker extends StatefulWidget {
  const QuickGridsPicker({super.key, required this.hWnd});

  final int hWnd;

  @override
  State<QuickGridsPicker> createState() => _QuickGridsPickerState();
}

class _QuickGridsPickerState extends State<QuickGridsPicker> {
  ...
  Widget build(BuildContext context) {
    if (_selectedPreset != null || _presets.length == 1) {
      ...
      Globals.lastQuickSnapZoneId = _selectedPreset!.id;
      return _ZoneGrid(
        preset: _selectedPreset!,
        onZoneTapped: _applyZone,
        onBack: () {
          Globals.lastQuickSnapZoneId = "";
          setState(() => _selectedPreset = null);
        },
      );
    } else if (Globals.lastQuickSnapZoneId != "") {
      if (_presets.any((QuickGrid element) => element.id == Globals.lastQuickSnapZoneId)) {
        _selectedPreset = _presets.firstWhere((QuickGrid element) => element.id == Globals.lastQuickSnapZoneId);
        return _ZoneGrid(
          preset: _selectedPreset!,
          onZoneTapped: _applyZone,
          onBack: () {
    ...
    return _PresetList(
      ...
      onPresetTapped: (QuickGrid preset) {
        Globals.lastQuickSnapZoneId = preset.id;
    ...
    return MouseRegion(
      ...
      child: InkWell(
        ...
        child: AnimatedContainer(
          ...
          color: _hov ? widget.accent.withValues(alpha: 0.08) : Colors.transparent,
            ...
            children: <Widget>[
              ...
              SizedBox(
                ...
                child: ClipRRect(
                  ...
                  child: CustomPaint(
                    painter: _MiniPainter(preset: widget.preset, accent: widget.accent),

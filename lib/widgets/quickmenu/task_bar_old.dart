...
class TaskBarState extends State<TaskBar> with QuickMenuTriggers, TabameListener {
  ...
  Future<void> handleHeight() async {
    ...
    double currentHeight = (windows.length * oneColumnHeight).clamp(100, 400) + 15;
  ...
  Future<void> handleAudio() async {
    ...
    if (audioMixer.isEmpty) {
      if (Caches.audioMixerExes.isEmpty) return;
      Caches.audioMixer.clear();
      Caches.audioMixerExes.clear();
      return;
    }
    Caches.audioMixer = audioMixer
        .where((ProcessVolume element) => element.peakVolume > 0.005)
        .map((ProcessVolume x) => x.processId)
        .toList();
                  ...
                  child: MouseRegion(
                    ...
                    child: Stack(
                      children: <Widget>[
                        Container(
                          width: 310,
                          decoration: BoxDecoration(color: _hoverElement == index ? (hoverColor) : Colors.transparent),
                          child: InkWell(
                            onTap: () {
                              ...
                              child: Padding(
                                ...
                                child: Wrap(
                                  ...
                                  children: <Widget>[
                                              ...
                                              child: (window.isPinned
                                                  ...
                                                  : ((Caches.audioMixer
                                                      ...
                                                      ? HoverScaleButton(
                                                          zoom: 1.8,
                                                          onTap: () async {
                                                            final List<ProcessVolume>? mixers =
                                                                await Audio.enumAudioMixer();
                                                            if (mixers == null) return;
                                                            for (int i = 0; i < mixers.length; i++) {
                                                              if (mixers[i].processPath == window.process.exePath) {
                                                                if (mixers[i].maxVolume.toStringAsFixed(2) == "0.00") {
                                                                  Audio.setAudioMixerVolume(mixers[i].processId, 1);
                                                                } else {
                                                                  Audio.setAudioMixerVolume(mixers[i].processId, 0.001);
                                                                }
                                                              }
                                                            }
                                                          },
                                    ...
                                    //2 Title
                                    SizedBox(
                                      width: index != _hoverElement ? 240 : 240 - hoverButtonsWidth + 10,
                                      child: Padding(
                          ...
                          Positioned(
                            ...
                            child: Container(
                              ...
                              constraints: BoxConstraints(
                                  minWidth: hoverButtonsWidth, maxWidth: hoverButtonsWidth, minHeight: oneColumnHeight),
                              child: Material(
                                type: MaterialType.transparency,
                                child: Wrap(
                                  children: <Widget>[
                                      ...
                                      Wrap(
                                        children: <Widget>[
                                          InkWell(
                                            ...
                                            child: GestureDetector(
                                                onSecondaryTap: () => WindowWatcher.mediaControl(index,
                                                    button: AppCommand.mediaNexttrack),
                                                onTertiaryTapUp: (TapUpDetails details) => WindowWatcher.mediaControl(
                                                    index,
                                                    button: AppCommand.mediaPrevioustrack),
                                    ...
                                    if (globalSettings.showMediaControlForApp &&
                                      ...
                                      InkWell(
                                        ...
                                        child: GestureDetector(
                                            ...
                                            onTertiaryTapUp: (TapUpDetails details) => WindowWatcher.mediaControl(index,
                                                button: AppCommand.mediaPrevioustrack),
                                            child: const SizedBox(
                                                width: 25,
                                                height: oneColumnHeight,
                                    ...
                                    InkWell(
                                      ...
                                      child: const SizedBox(
                                          width: 25, height: oneColumnHeight, child: Icon(Icons.close, size: 15)),
                                    ),
                                  ],
                                ),
                              ),
                  ...
                  return Column(children: <Widget>[
                    Container(
                      ...
                      decoration: BoxDecoration(
                          color: Colors.transparent, border: Border(top: BorderSide(width: 2, color: hoverColor))),
                    ),
                    el
                  ]);
                } else if (windows.length > 10 && index == windows.length - 1) {
                  return Column(
                    children: <Widget>[
                      el,
                      Container(height: 20),
                            ...
                            child: Row(
                              ...
                              children: <Widget>[
                                SizedBox(
                                    child: Padding(
                                  padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 0),
                                  child: Icon(Icons.pin_end_outlined,
                                      color: Theme.of(context).textTheme.bodyMedium?.color?.withValues(alpha: 0.6),
                                      size: 18),
                                )),
                                Expanded(
                                    child: Text(window.isPinned ? "Unpin" : 'Set Always on Top',
                                        style: Theme.of(context).textTheme.labelLarge?.copyWith(height: 1))),
                              ],
                            ),
                          ),

...
class FontPickerUI extends StatefulWidget {
  ...
  @override
  State<FontPickerUI> createState() => _FontPickerUIState();
}

class _FontPickerUIState extends State<FontPickerUI> {
  List<PickerFont> _shownFonts = <PickerFont>[];
  List<PickerFont> _allFonts = <PickerFont>[];
  List<PickerFont> _recentFonts = <PickerFont>[];
  ...
  final List<String> googleFontsList = <String>[
    'ABeeZee',
    'Abel',
    'Abhaya Libre',
    'Aboreto',
  ...
  Future _prepareShownFonts() async {
    ...
    var recents = prefs.getStringList(prefsRecentsKey) ?? <dynamic>[];
    final List<String> supportedFonts =
        GoogleFonts.asMap().keys.where((String e) => googleFontsList.contains(e)).toList();
    setState(() {
      _recentFonts = recents.reversed.map((fontFamily) => PickerFont(fontFamily: fontFamily, isRecent: true)).toList();
      _allFonts = _recentFonts +
          supportedFonts
              .where((String fontFamily) => !recents.contains(fontFamily))
              .map((String fontFamily) => PickerFont(fontFamily: fontFamily))
              .toList();
                  ...
                  return ListTile(
                    ...
                    title: Padding(
                      ...
                      child: RichText(
                        text: TextSpan(
                          text: f.fontFamily,
                          style: TextStyle(
                            fontFamily: GoogleFonts.getFont(f.fontFamily).fontFamily,
                          ).copyWith(
                    ...
                    subtitle: isBeingSelected
                        ? Padding(
                            ...
                            child: Wrap(
                              children: ["300", "400", "500", "600", "700", "800"].map((String variant) {
                                bool isSelectedVariant;
                                isSelectedVariant = variant == "italic" && _selectedFontStyle == FontStyle.italic
                                    ? true
                                  ...
                                  child: Padding(
                                    ...
                                    child: TextButton(
                                      style: TextButton.styleFrom(
                                        backgroundColor: isSelectedVariant ? Theme.of(context).primaryColor : null,
                                      ),
                                      child: Center(
                                        child: Text(
                                          variant,
                                      ...
                                      onPressed: () {
                                        setState(() {
                                          if (variant == "italic") {
                                            ...
                                            _selectedFontWeight = fontWeightValues[variant]!;
                                          }
                                        });
                                      },
                                    ),
  ...
  Future<void> addToRecents(String fontFamily) async {
    ...
    if (recents != null && !recents.contains(fontFamily)) {
      ...
      prefs.setStringList(prefsRecentsKey, <String>[fontFamily]);
    }
  }
  ...
  void onSearchTextChanged(String text) {
    if (text.isEmpty) {
      ...
      setState(() {
        _shownFonts = _allFonts
            .where(
              (PickerFont f) => f.fontFamily.toLowerCase().contains(
                    text.toLowerCase(),
                  ),
            )
            .toList();

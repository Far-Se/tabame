...
class QuickGridsSettingsPage extends StatefulWidget {
  const QuickGridsSettingsPage({super.key});

  @override
  State<QuickGridsSettingsPage> createState() => _QuickGridsSettingsPageState();
}

class _QuickGridsSettingsPageState extends State<QuickGridsSettingsPage> {
            ...
            children: <Widget>[
              Expanded(
                child: Column(
                  ...
                  children: <Widget>[
                    Text('QuickSnap Zones', style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w700)),
...
class _ZoneFreestyleCanvasState extends State<_ZoneFreestyleCanvas> {
  ...
  static const double kSnapThr = 0.03;
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          Row(
            children: <Widget>[
              _PctField(
                key: ValueKey<String>('L-$zoneIndex-${lPct.round()}'),
                label: 'Left',
                value: lPct,
              ...
              const SizedBox(width: 8),
              _PctField(
                key: ValueKey<String>('T-$zoneIndex-${tPct.round()}'),
                label: 'Top',
                value: tPct,
              ...
              const SizedBox(width: 8),
              _PctField(
                key: ValueKey<String>('W-$zoneIndex-${wPct.round()}'),
                label: 'Width',
                value: wPct,
                onChanged: (double v) =>
              ...
              const SizedBox(width: 8),
              _PctField(
                key: ValueKey<String>('H-$zoneIndex-${hPct.round()}'),
                label: 'Height',
                value: hPct,
                onChanged: (double v) =>
...
class _PctFieldState extends State<_PctField> {
  ...
  void didUpdateWidget(_PctField old) {
    ...
    if (!_focused) {
      final double parsed = double.tryParse(_ctrl.text) ?? -999;
      if ((parsed - widget.value).abs() > 0.15) {

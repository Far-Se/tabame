...
class HotkeyHandler {
  ...
  int currentVK = -1;
  Future<void> handle(HotkeyEvent hotkeyInfo) async {
        ...
        if (list.isNotEmpty) {
          for (KeyMap key in list) {
            ...
            if ((startMouseDir == 1 &&
                    key.triggerInfo[0] == 0 &&
                    diffAux.x < 0 &&
                    diffAux.x.abs() > key.triggerInfo[1]) ||
                ((startMouseDir == 1 &&
                    key.triggerInfo[0] == 1 &&
                    diffAux.x > 0 &&
                    diffAux.x.abs() > key.triggerInfo[1])) ||
                ((startMouseDir == 2 &&
                    key.triggerInfo[0] == 2 &&
                    diffAux.y < 0 &&
                    diffAux.y.abs() > key.triggerInfo[1])) ||
                ((startMouseDir == 2 &&
                    key.triggerInfo[0] == 3 &&
                    diffAux.y > 0 &&
                    diffAux.y.abs() > key.triggerInfo[1]))) {
    ...
    if (hotkeyInfo.action == "released") {
      ...
      if (mouseDir.isNotEmpty) {
        ...
        for (KeyMap key in mouseDir) {
          ...
          if ((key.triggerInfo[0] == 0 && diff.x < 0 && diffX.isBetweenEqual(key.triggerInfo[1], key.triggerInfo[2])) ||
              ((key.triggerInfo[0] == 1 &&
                  diff.x > 0 &&
                  diffX.isBetweenEqual(key.triggerInfo[1], key.triggerInfo[2]))) ||
              ((key.triggerInfo[0] == 2 &&
                  diff.y < 0 &&
                  diffY.isBetweenEqual(key.triggerInfo[1], key.triggerInfo[2]))) ||
              ((key.triggerInfo[0] == 3 &&
                  diff.y > 0 &&
                  diffY.isBetweenEqual(key.triggerInfo[1], key.triggerInfo[2])))) {
            await key.applyActions();
      ...
      if (hotkeyDoublePress.containsKey(hotkey.hotkey) && hotkeyInfo.name.isNotEmpty) {
        keys = hotkey.keymaps
            .where((KeyMap element) => element.boundToRegion && element.triggerType == TriggerType.doublePress)
            .toList();
        for (KeyMap key in keys) {
      ...
      keys = hotkey.keymaps
          .where((KeyMap element) => element.boundToRegion && element.triggerType == TriggerType.press)
          .toList();

...
| ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| Start with Windows                               | Off by default; current implementation uses a per-user Startup `.lnk`                                                                         | User enables the setting; must be reversible                                                                            | **Allowed** via per-user Startup-folder shortcut or HKCU Run; startup elevation remains opt-in                               | If registration is blocked, show unavailable/error state and link to Windows Startup settings                                             | “Starts Tabame when you sign in”; disclose that Elevated Permission can request UAC on each launch                                        | `STORE-START-01`: opt in, sign out/in, disable, verify normal startup by default and no duplicate entry                            |
| Administrator-only actions / elevation           | Off by default; `storeInstaller` also supports an explicit persisted Elevated Permission preference                                           | User invokes **Elevated Permission** for startup, **Restart elevated for this session**, or a named selected-app action | **Allowed with redesign** for `storeInstaller`; explicit UAC is required for every opted-in startup or selected operation    | Continue at standard integrity if startup UAC is denied; hide or reject elevation in `storeMsix` because `allowElevation` is not approved | Explain why elevation is needed, that startup elevation is opt-in and prompts on each launch, and that cancellation leaves Tabame running | `STORE-ELEV-01`: standard start, medium-integrity administrator account, accept/deny/cancel UAC, verify opt-in startup replacement |
...
- silent or unconsented administrator relaunch at startup;

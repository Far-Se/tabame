...
| ------------------- | ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Windows             | ZIP plus `tabame-nightly-windows-installer.exe` / `tabame-v<version>-windows-installer.exe` Inno assets   | Existing Windows workflow and Windows 10/11-era native integrations | Extract the ZIP and run `tabame.exe`, or run the per-user Inno installer under `%LOCALAPPDATA%\Programs\Tabame` | Run a newer installer over the existing install, or stop Tabame before replacing a ZIP install; `%LOCALAPPDATA%\Tabame` is retained |
...
- Official builds still upload `tabame-v<version>-windows.zip` plus its
  SHA-256 file.
- The dependent `build-windows-installer` job also builds the
  `storeInstaller` profile with Inno Setup and attaches the versioned installer,
  checksum, manifest, SBOM, and metadata to the same GitHub Release. The
  installer keeps a stable AppId/install path, closes a running Tabame process
  through Windows Restart Manager, and preserves `%LOCALAPPDATA%\Tabame`.

...
| `copyImage`        | `url` (HTTP(S)) or `path`/`file` (local image)                                   | Downloads/loads image pixels into the image clipboard; Windows currently.                                                                                                                                                   |
...
Notes: combine effects with multiple lines (`copy`+`hide` = copy-and-dismiss, but toast is skipped since launcher is already gone). `copyImage` accepts an HTTP(S) image URL, a `file://` URL, or a local path (relative paths use the plugin folder); it copies pixels, not the URL. `copyFile` copies local file/folder references and accepts `paths` for multiple entries.
...
```jsonc
{
  ...
  "depth": 1,
  "expanded": true, // tree indentation/current disclosure state
  ...
  "end": "2026-08-04T10:15:00", // calendar; `date` alias + allDay/color/location supported
  "media": {
...
window extension plugin refresh sync gamepad game book note run open reply pin
```

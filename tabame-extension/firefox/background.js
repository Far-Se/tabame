...
async function readBridgeConfig() {
  const stored = await browser.storage.local.get(["bridgeToken", "bridgePort"]);
  return {
    token:
...
function setBadge(connected) {
  ...
  browser.action.setBadgeText({ text: connected ? "ON" : "" });
...
async function dispatchRequest(method, params) {
  switch (method) {
    ...
    case "tabs.close":
      await browser.tabs.remove(requireTabId(params));
      return { closed: true };
    case "tabs.mute":
    ...
    case "tabs.reload":
      await browser.tabs.reload(requireTabId(params));
      return { reloaded: true };
    case "tabs.duplicate":
      return serializeTab(await browser.tabs.duplicate(requireTabId(params)));
    case "tabs.open":
...
async function activateTab(tabId) {
  ...
  return serializeTab(await browser.tabs.update(tabId, { active: true }));
}
...
async function setTabMuted(tabId, requestedState) {
  ...
  return serializeTab(await browser.tabs.update(tabId, { muted }));
}
...
async function setTabPinned(tabId, requestedState) {
  ...
  return serializeTab(await browser.tabs.update(tabId, { pinned }));
}

async function openTab(url, active) {
  return serializeTab(
    await browser.tabs.create({ url: safeWebUrl(url), active: Boolean(active) }),
  );
}

async function resolveScriptTab(params) {
  if (params.tabId != null) {
    return browser.tabs.get(requireTabId(params));
  }
...
async function ensureUserScriptsAvailable() {
  if (!browser.userScripts || typeof browser.userScripts.execute !== "function") {
    throw new Error(
      "JavaScript execution is disabled. Grant the optional \"userScripts\" permission from this extension's Firefox permissions.",
    );
  }
  try {
...
async function executeJavascript(params) {
  ...
  const injectionResults = await browser.userScripts.execute(injection);
...
browser.tabs.onActivated.addListener(() => queueTabEvent("activated"));
browser.tabs.onCreated.addListener(() => queueTabEvent("created"));
browser.tabs.onRemoved.addListener(() => queueTabEvent("removed"));
...
browser.tabGroups.onCreated.addListener(() => queueTabEvent("group-created"));
browser.tabGroups.onMoved.addListener(() => queueTabEvent("group-moved"));
browser.tabGroups.onRemoved.addListener(() => queueTabEvent("group-removed"));
browser.tabGroups.onUpdated.addListener(() => queueTabEvent("group-updated"));
...
browser.runtime.onMessage.addListener((message) => {
  ...
  if (message.action === "status") {
    return Promise.all([readBridgeConfig(), userScriptsStatus()]).then(
      ([config, scripts]) => ({

...
function runtimeMessage(message) {
  return browser.runtime.sendMessage({ type: "popup", ...message });
}
...
async function requestUserScriptsPermission() {
  ...
  try {
    // Firefox exposes userScripts as an optional-only permission and requires
    // this request to originate from a user action in the popup.
  ...
  if (status.connected) {
    ...
    statusDetail.textContent = status.userScriptsAvailable
      ...
      : status.userScriptsError ||
        'Grant the optional "userScripts" permission in Firefox.';
    return status;
  }
...
pairingForm.addEventListener("submit", async (event) => {
  ...
  await requestUserScriptsPermission();
